...
statspath := 1;
	...
	begin
		...
		// Display
		gotoxy(2,5); write('   ',#179,'',#136,' _');
		gotoxy(2,6); write('   ',#197,'O\',#206);
		gotoxy(2,7); write('   / )',#202,' ');
		gotoxy(2,10);
		writeln(select[1],' Warrior ');
		gotoxy(16,5); write('  ',#143,' ',#245);
		gotoxy(16,6); write(' /H\',#197);
		gotoxy(16,7); write(' /_\',#179);
		gotoxy(16,10);
		writeln(select[2],' Mage ');
		gotoxy(30,5); write('   õ');
		gotoxy(30,6); write(' '#170'/O\i');
		gotoxy(30,7); write('  / \ ');
		gotoxy(30,10);
...
Begin
    ...
    gotoxy(3,20); write('                                                                             `Y8P"                                                        ');
		...
		gotoxy(3,22); write(select[1],' Iniciar');
		gotoxy(3,24); write(select[2],' Defenições');
		gotoxy(3,26); write(select[3],' Sair');
...
Begin
	...
	Begin
		textcolor(yellow);
		gotoxy(52,8); write('KeyBinds:');
		gotoxy(52,11); write('wasd - Andar');
		gotoxy(52,12); write('"Tab" - Abrir a loja de pontos');
		gotoxy(52,24); write('Pressione "p" para sair.');
