...
Begin
		...
		textcolor(cyan);
		gotoxy(84,20); write('Controlos:');
		delay(1000);
		gotoxy(84,23); write('W - Andar para cima ');
		gotoxy(84,24); write('A - Andar para baixo');
		gotoxy(84,25); write('S - Andar para a esquerda');
		gotoxy(84,26); write('D - Andar para a direita');
		gotoxy(84,28); write('Não podes usar as ',#27,' ',#25,' ',#24,' ',#26,' em vez do WASD');
		delay(2000);
		gotoxy(84,30); write('"Tab" - Abrir a loja de pontos');
		delay(2000);
		gotoxy(84,32); write('"P" - Para aceder aos comandos durante o jogo');
		delay(2000);
		gotoxy(84,34); write('"ENTER" - Para continuar');
...
Begin
	...
	gotoxy(85,43);  write(select[4],'Poção de Cura (',itemCura,')');
	gotoxy(115,43);  write(select[5],'Poção de Dano (',itemDano,')');
	gotoxy(160,43); write(select[6],'Fugir ');			
			...
			#13: begin
					...
					if(fightpath < 4) then
						...
						if(bossAttack = 1) then
							begin
								gotoxy(157,35); write('O oponente usou Bafo de Fogo');
								playerHealth := playerHealth - 4;		
							end;
			
						if(bossAttack = 2) then
							begin
								gotoxy(157,35); write('O oponente usou Patada');
								playerHealth := playerHealth - 6;		
							end;

						if(bossAttack = 3) then
							begin
								gotoxy(157,35); write('O oponente falhou o ataque');		
							end;		
...
Begin
	...
	Begin
		...
		gotoxy(30,13); write('A - Andar para baixo');
		gotoxy(30,14); write('S - Andar para a esquerda');
		gotoxy(30,15); write('D - Andar para a direita');
		gotoxy(30,27); write('Não podes usar as ',#27,' ',#25,' ',#24,' ',#26,' em vez do WASD');
		
		gotoxy(30,19); write('"Tab" - Abrir a loja de pontos');
...
Begin
	...
	Begin
		...
		fightObject(45,10,8,5,#219,4);
		fightObject(101,23,8,5,#219,4);
		fightObject(13,39,8,5,#219,4);
		fightObject(151,10,8,5,#219,4);
		fightObject(151,22,8,5,#219,4);
		fightObject(191,18,8,5,#219,4);
